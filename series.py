...
class Series:
    ...
    def make_key(self,filename,volume_idx,layer_names):
        if volume_idx is None:
            ...
            temp = volume_idx
        return '%s_%06d_%s'%(filename.replace(os.sep,'_'),temp,'_'.join(layer_names))
    ...
    def map_into_reference_space(self,filename,vidx,layer_names,goodness_threshold=-np.inf,oversample_factor=1.0,oversample_method='nearest'):
        ...
        if vidx==None:
            target_data = target_hive['processed_data'][:,:,:]
            target_data_time = target_hive['data_time'][:,:]
        else:
    ...
    def add(self,filename,vidx,key=None,layer_names=None,overwrite=True,oversample_factor=3,strip_width=3.0,do_plot=False,use_gaussian=False,background_diameter=0,refine=False,graph=False,left_crop=None,right_crop=None,rb_prereg=False,rb_xmax=5,rb_ymax=5):
        ...
        if vidx is None:
            ...
            temp = vidx
        self.db.put(self.key,filename,temp)
    ...
    def get_image(self,filename,vidx,layer_names):
        ...
        if vidx is None:
            ...
            if len(layer_names)>1:
                print target_hive['projections'][layer_names[0]][:,:]

                test = target_hive['projections'][layer_names[0]][:,:]
                n_slow,n_fast = test.shape
                stack = np.zeros((len(layer_names),n_slow,n_fast))
                for idx,layer_name in enumerate(layer_names):
                    stack[idx,:,:] = target_hive['projections'][layer_name][:,:]
    ...
    def render(self,layer_names=None,goodness_threshold=0.0,correlation_threshold=-1.0,overwrite=False,oversample_factor=3,do_plot=False,make_movie=False,left_crop=0,right_crop=0):
        ...
        for k_idx,k in enumerate(keys):
            ...
            try:
                ...
                test,label = self.get_image(filename,None,layer_names)
        ...
        for k in sorted(reg_dict.keys()):
            ...
            try:
                ...
                filename = temp[0]
                frame_index = 0
                im,label = self.get_image(filename,None,layer_names)
        ...
        if do_plot:
            ...
            clim = np.percentile(self.reference,(1,99.5))
            plt.subplot(1,3,1)
            plt.imshow(self.reference,cmap='gray',interpolation='none',clim=clim)
            plt.title('reference image')
            
            plt.subplot(1,3,2)
            plt.imshow(av,cmap='gray',interpolation='none',clim=clim)
            plt.title('registered average (cropped to ref)')
            plt.xlim((ref_x1,ref_x2))
            plt.ylim((ref_y2,ref_y1))
            
            plt.subplot(1,3,3)
            plt.imshow(counter_image)
            plt.title('number of contributing strips (cropped)')
            plt.colorbar()
            plt.xlim((ref_x1,ref_x2))
            plt.ylim((ref_y2,ref_y1))
